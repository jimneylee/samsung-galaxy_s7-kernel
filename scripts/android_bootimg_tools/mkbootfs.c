...
static void _eject(struct stat *s, char *out, int olen, char *data, unsigned 
datasize)
{
...
static void _archive_dir(char *in, char *out, int ilen, int olen)
{
    ...
    while((de = readdir(d)) != 0){
        ...
        if(!strcmp(de->d_name, "default.prop.init")) continue;
...
static void read_canned_config(char* filename)
{
    ...
    int used = 0;
    canned_config =
        (struct fs_config_entry*)malloc(allocated * sizeof(struct 
fs_config_entry));
