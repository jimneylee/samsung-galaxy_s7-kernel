...
static void mkpath(const char *path) {
    ...
    // check if path length exceeds PATH_MAX
    // also we should reserve 128 chars for filename
    len = strlen(path);
    if (len > (PATH_MAX - 128)) {
        puts("mkpath: the specified path is too long.");
        exit(1);
    }
...
int main(int argc, char** argv)
{
    ...
    while(argc > 0){
        ...
        if(!strcmp(arg, "--input") || !strcmp(arg, "-i")) {
            ...
            if (strlen(basename(filename)) > (128 - 16)) {
                puts("The name of your input file is too long.");
                exit(1);
            }
