...
static inline uint16_t get2LE(const uint8_t* src) { return src[0] | (src[1] << 
8); }
...
"vendor/bin/hw/android.hardware.wifi@1.0-service" },
    ...
    { 00644, AID_ROOT,      AID_ROOT,      0, "lib64/*" },
    { 00644, AID_ROOT,      AID_ROOT,      0, "res/*" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/adbd" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/bgcompact" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/cbd" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/dev_guardian" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/emmc_quickverify" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/ffu" },
    { 00640, AID_ROOT,      AID_SHELL,     0, "sbin/firmware_key.cer" },
    { 00750, AID_ROOT,      AID_SHELL,     0, "sbin/healthd" },
...
static size_t strip(const char* path, size_t len, const char suffix[]) {
    ...
    if (strncmp(path + len - strlen(suffix), suffix, strlen(suffix))) return 
len;
...
*/
        ...
        if (asprintf(&name, "%.*s%s", (int)len, target_out_path, 
conf[which][dir]) != -1) {
...
static bool fs_config_cmp(bool dir, const char* prefix, size_t len, const char* 
path, size_t plen) {
    if (dir) {
        if (plen < len) {
            return false;
...
sizeof(header)) {
            ...
            if (remainder <= 0) {
                fprintf(stderr, "%s len is corrupted\n", conf[which][dir]);
                break;
            }
            ...
            if (TEMP_FAILURE_RETRY(read(fd, prefix, remainder)) != remainder) {
                free(prefix);
                fprintf(stderr, "%s prefix is truncated\n", conf[which][dir]);
                break;
            }
...
fs_path_config* pc) {
    struct fs_path_config_from_file* p = (struct 
fs_path_config_from_file*)buffer;
